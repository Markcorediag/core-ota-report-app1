...
def detect_brand_column(df: pd.DataFrame) -> Optional[str]:
    ...
    candidates = [
        "Vehicle Path", "Brand", "Make", "Manufacturer", "VEHICLE PATH", "MAKE", "BRAND",
    ]
    for c in candidates:
        if c in df.columns:
            return c
...
def extract_model_series(df: pd.DataFrame, brand_col: str) -> pd.Series:
    """Extract Model from combined column (after '|', before ' - '),
    or use a dedicated 'Model' column if present."""
...
def build_pdf(
    ...
    if df_brand_models.empty:
        ...
        if one_page_mode:
            intro_penalty = max(0, len(intro) // 180)  # every ~180 chars ≈ 1 table row of space
        ...
        table.setStyle(TableStyle([
            ...
            ("ALIGN", (1, 1), (2, -1), "LEFT"),  # left-align Brand & Model cells
